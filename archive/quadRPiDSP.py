"""
quadRPiDSP.py - Multi-Matrix Quadraphonic Digital Signal Processor

(Raspberyy Pi 5 version)
...
formats (SQ, QS ) from a 2-channel 
...
while True:
  ...
  try:
    while True:
        # 1. KONTROLA SOUBORU (Příkazy z Kodi/Terminálu)
        if os.path.exists(CMD_FILE):
            try:
                ...
                elif cmd == 'q' or cmd == 'mode:sq': mode = "sq"
                elif cmd == 'x' or cmd == 'mode:qs': mode = "qs"
                elif cmd == 's' or cmd == 'mode:stereo': mode = "stereo"
                elif cmd == 'f' or cmd == 'toggle:filter': enabled_filter = not enabled_filter
                elif cmd == 'r': total_clicks = 0
                elif cmd == 'restart': start_engine()
                elif cmd == 'e' or cmd == 'exit': break
                
                # Po každém příkazu překreslíme menu v logu
                print_menu()
                
            except Exception as e:
                print(f"\nChyba zpracování příkazu: {e}")

        # 2. NEBLOKUJÍCÍ ČEKÁNÍ
        # sd.sleep(200) uspí hlavní vlákno na 0.2s, 
        # ale audio callback v jiném vlákně běží dál!
        sd.sleep(200)
